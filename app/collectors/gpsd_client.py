...
def collect(host=None, port=None, timeout=_READ_TIMEOUT):
    """
    Connect to gpsd once, read both TPV and SKY messages, return combined dict.

    Using a single connection for both is more efficient than two separate
    connections — gpsd streams both message types continuously once ?WATCH
    is sent.
    ...
    try:
        with _GpsdReader(host, port, timeout=timeout + 2) as r:
            messages = r.read_classes(["TPV", "SKY"], timeout=timeout)
        result["tpv"] = _parse_tpv(messages["TPV"])
        result["sky"] = _parse_sky(messages["SKY"])
    except (OSError, ConnectionError) as e:
...
if __name__ == "__main__":
    ...
    # Verify expected constellations present (GPS and BeiDou confirmed on PowerPi)
